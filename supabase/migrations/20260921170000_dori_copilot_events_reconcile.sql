...
CREATE TABLE IF NOT EXISTS public.dori_decision_events (
    id uuid PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS dori_decision_driver_evaluated_idx
ON public.dori_decision_events(driver_profile_id, evaluated_at DESC, id DESC);

CREATE INDEX IF NOT EXISTS dori_decision_station_evaluated_idx
ON public.dori_decision_events(station_id, evaluated_at DESC, id DESC);

CREATE INDEX IF NOT EXISTS dori_decision_versions_idx
ON public.dori_decision_events(model_version, rules_version, parameter_version);

CREATE TABLE IF NOT EXISTS public.dori_outcome_events (
    id uuid PRIMARY KEY,
    environment_id uuid NOT NULL,
    station_id uuid NOT NULL,
    driver_profile_id uuid NOT NULL,
...
CREATE INDEX IF NOT EXISTS dori_outcome_driver_observed_idx
ON public.dori_outcome_events(driver_profile_id, observed_at DESC, id DESC);

CREATE INDEX IF NOT EXISTS dori_outcome_station_observed_idx
...
DROP POLICY IF EXISTS dori_decision_authorized_read ON public.dori_decision_events;
...
DROP POLICY IF EXISTS dori_outcome_authorized_read ON public.dori_outcome_events;
...
DROP TRIGGER IF EXISTS dori_decision_block_update_delete ON public.dori_decision_events;
...
DROP TRIGGER IF EXISTS dori_outcome_block_update_delete ON public.dori_outcome_events;
