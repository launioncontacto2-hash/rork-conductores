...
DECLARE actor uuid:=app.auth_profile_id(); env uuid:=app.current_environment_id(); station uuid; existing public.command_log%ROWTYPE; cmd uuid; case_id uuid; result jsonb;
BEGIN
  ...
  IF p_input_payload->>'source' <> 'simulated' OR p_input_payload->>'driver' IS NULL THEN RAISE EXCEPTION 'simulation_payload_required' USING ERRCODE='22023'; END IF;
  SELECT * INTO existing FROM public.command_log WHERE environment_id=env AND idempotency_key=btrim(p_idempotency_key);
  IF FOUND THEN RETURN existing.result_payload; END IF;
  INSERT INTO public.command_log(environment_id,actor_profile_id,command_name,idempotency_key,request_payload,status) VALUES(env,actor,'console_create_dori_simulation_case',btrim(p_idempotency_key),jsonb_build_object('test_case_id',p_test_case_id,'driver_profile_id',p_driver_profile_id,'input_payload',p_input_payload,'expected_recommendation',p_expected_recommendation,'expires_at',p_expires_at),'accepted') RETURNING id INTO cmd;
