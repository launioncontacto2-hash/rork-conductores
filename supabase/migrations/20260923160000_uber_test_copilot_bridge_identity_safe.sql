-- Identity-safe correction for UBER Test -> Copiloto bridge.
...
begin
  select e.* into v_event
   ...
   where e.batch_id=p_batch_id
     and d.profile_id=(select app.auth_profile_id())
...
begin
  ...
  select o.* into strict v_offer from public.uber_test_offers o join public.uber_test_offer_batches b on b.id=o.batch_id join public.driver_profiles d on d.id=b.driver_profile_id where o.id=p_offer_id and d.profile_id=(select app.auth_profile_id()) and b.environment='TEST';
...
begin
  select b.* into v_batch from public.uber_test_offer_batches b join public.driver_profiles d on d.id=b.driver_profile_id
   where d.profile_id=(select app.auth_profile_id()) and b.environment='TEST' and b.status in ('queued','active') order by b.created_at asc limit 1;
