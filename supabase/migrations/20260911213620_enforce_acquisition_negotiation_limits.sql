-- El limite comercial es autoritativo en backend. El cliente solo lo refleja.
...
DECLARE
    ...
    v_counteroffer_count integer;
    v_request jsonb;
    v_result jsonb;
    v_command public.command_log%ROWTYPE;
BEGIN
    IF p_action NOT IN ('counteroffer', 'accept', 'award', 'reject', 'withdraw') THEN
    ...
    FOR UPDATE;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'acquisition_offer_not_found' USING ERRCODE = 'P0002';
    END IF;
    IF NOT (
        v_role = 'dori_admin'
        OR (v_role = 'provider' AND v_offer.supplier_id = v_supplier_id)
    ) THEN
    ...
    v_command := app.begin_acquisition_command(
        'respond_acquisition_offer.' || p_action,
        p_idempotency_key,
        v_request
    );
    ...
    IF p_action = 'counteroffer' THEN
        IF p_amount_mxn IS NULL OR p_amount_mxn <= 0
           OR v_offer.status NOT IN ('submitted', 'negotiating') THEN
            RAISE EXCEPTION 'invalid_acquisition_counteroffer' USING ERRCODE = '22023';
        END IF;

        SELECT count(*) INTO v_counteroffer_count
        ...
        WHERE negotiation.offer_id = p_offer_id
          ...
          AND negotiation.actor_role = v_role
          AND negotiation.action = 'counteroffer';

        IF v_counteroffer_count >= 2 THEN
            RAISE EXCEPTION 'acquisition_counteroffer_limit_reached' USING ERRCODE = '55000';
        END IF;
        ...
        v_result := jsonb_build_object(
            'offer_id', p_offer_id,
            'status', 'negotiating',
            'counteroffers_used', v_counteroffer_count + 1,
            'counteroffers_limit', 2
        );
    ...
    ELSIF p_action = 'accept' THEN
        ...
        IF NOT FOUND OR v_last.actor_profile_id = v_actor_profile_id
           OR v_offer.status <> 'negotiating' THEN
        ...
        v_result := jsonb_build_object(
            'offer_id', p_offer_id,
            'status', 'price_agreed',
            'agreed_price_mxn', v_last.amount_mxn
        );

    ELSIF p_action = 'reject' THEN
        IF v_role <> 'dori_admin'
           OR v_offer.status NOT IN ('submitted', 'negotiating', 'price_agreed') THEN
    ...
    ELSIF p_action = 'withdraw' THEN
        IF v_role <> 'provider'
           OR v_offer.status NOT IN ('submitted', 'negotiating', 'price_agreed') THEN
            RAISE EXCEPTION 'provider_withdraw_not_allowed' USING ERRCODE = '42501';
        END IF;
        UPDATE public.acquisition_offers
        SET status = 'withdrawn', revision = revision + 1, updated_at = v_now
        WHERE id = p_offer_id;
        v_result := jsonb_build_object('offer_id', p_offer_id, 'status', 'withdrawn');
    ...
    ELSE
        ...
        INSERT INTO public.acquisition_orders(
            ...
            v_environment_id,
            'OC-' || upper(substr(replace(gen_random_uuid()::text, '-', ''), 1, 10)),
        ...
        SET status = CASE
                WHEN (
                    ...
                    JOIN public.acquisition_offers awarded_offer
                      ON awarded_offer.id = acquisition_order.offer_id
...
COMMENT ON FUNCTION public.respond_acquisition_offer(uuid, text, numeric, text, text) IS
    'Negociacion transaccional: maximo dos contraofertas por parte; aceptar, cerrar o adjudicar permanecen en RPC.';
