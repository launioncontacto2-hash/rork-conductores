...
DO $do$ DECLARE d text; BEGIN
  d:=pg_get_functiondef('public.console_audit_history(integer)'::regprocedure);
  d:=replace(d,'m.role = ''supervisor''','m.role IN (''supervisor'',''console'')');
  d:=replace(d,'app.auth_has_role(''supervisor'', event.station_id)','app.auth_can_operate_station(event.station_id)'); EXECUTE d;
END $do$;
