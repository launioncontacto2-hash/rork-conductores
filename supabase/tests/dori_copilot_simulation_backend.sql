...
SELECT ok(pg_get_functiondef('public.driver_next_dori_simulation_case(uuid)'::regprocedure) LIKE '%expires_at>now()%' AND pg_get_functiondef('public.driver_next_dori_simulation_case(uuid)'::regprocedure) LIKE '%ORDER BY created_at ASC, id ASC%','next usa reloj real y FIFO determinista');
SELECT ok(pg_get_functiondef('public.console_create_dori_simulation_case(uuid,text,text,uuid,jsonb,text,text,text,timestamptz)'::regprocedure) LIKE '%created_at,expires_at)% VALUES%now(),p_expires_at%','create registra created_at con reloj real');
SELECT ok(pg_get_functiondef('public.driver_get_dori_simulation_case(uuid,uuid)'::regprocedure) LIKE '%c.expires_at<=now()%' AND pg_get_functiondef('public.record_dori_simulation_evaluation(uuid,uuid,text,jsonb)'::regprocedure) LIKE '%c.expires_at<=now()%','get/evaluate expiran con reloj real');
