...
enum SupabaseFinancialService {
    ...
    nonisolated struct RegisterIncomeParameters: Encodable, Sendable {
        ...
        /// PostgREST resolves overloaded RPCs from the complete set of JSON keys. Swift's
        /// synthesized `Encodable` omits nil optionals, which made `p_evidence_path` and
        /// `p_reversal_of` disappear and left no matching `register_income` signature in
        /// the schema cache. Encode every nullable SQL argument explicitly as JSON null.
