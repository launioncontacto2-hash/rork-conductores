...
public final class UberTestReceiverState: ObservableObject {
    ...
    private func call(_ function: String, token: @escaping @Sendable () async -> String?, refresh: (@Sendable () async -> String?)?, body: [String: String]) async {
        ...
        guard let baseURL, let url = URL(string: "rest/v1/rpc/\(function)", relativeTo: baseURL) else { return }
        ...
        if (response as? HTTPURLResponse)?.statusCode == 401, let refresh, let refreshed = await refresh() { request.setValue("Bearer \(refreshed)", forHTTPHeaderField: "Authorization"); request.setValue(refreshed, forHTTPHeaderField: "apikey"); (data, response) = (try? await URLSession.shared.data(for: request)) ?? (Data(), URLResponse()) }
